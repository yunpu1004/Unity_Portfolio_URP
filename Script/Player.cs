...
public class Player : MonoBehaviour
{
    ...
    private void CheckInput()
    {
        ...
        if(dialogueUI.onDialogue || tradeUI.onInteraction || inventoryUI.isInventoryOpen)
        {
            input.IgnoreCharacterMovement();
            input.IgnoreInteract();
            input.IgnoreTapMenu();
        ...
        if(input.skillE && input.skillQ)
        {
            if(skill.GetSkillECooltime() > 0) input.IgnoreSkillE();
            else input.IgnoreSkillQ();
        }
    ...
    {
        Vector3 targetPosition = new Vector3(-92.13f, 0.0592f, 27.625f);
