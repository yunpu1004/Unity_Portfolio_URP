...
public class PlayerStatUI : MonoBehaviour
{
    ...
    private void Awake() 
    {
        ...
        playerStat.AddOnLevelChangedEvent(SetLevelText);
        playerStat.AddOnHPChangedEvent((hp, maxHp) => SetMaxHpText(maxHp));
        playerStat.AddOnAtkChangedEvent(SetAtkText);
        playerStat.AddOnDefChangedEvent(SetDefText);
        playerStat.AddOnWeaponChangedEvent(weaponSlot.UpdateSlot);
        playerStat.AddOnShieldChangedEvent(shieldSlot.UpdateSlot);
    ...
    // 공격력 텍스트를 업데이트합니다.
    private void SetAtkText(int value)
    {
        atkText.text = value.ToString();
    }

    // 방어력 텍스트를 업데이트합니다.
    private void SetDefText(int value)
    {
        defText.text = value.ToString();
    }

    // 최대 체력 텍스트를 업데이트합니다.
    private void SetMaxHpText(int maxHp)
    {
